...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    //#Section -> Copiado de ItinerarioActivity
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET,
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        PolylineOptions polLineOpt = new PolylineOptions();
                        int i = 1; //Int for name
                        for(Itinerario.LatLng latLng : itinerario.positions){
                            LatLng marker = new LatLng(latLng.lat, latLng.lng);
                            if(i == 1){
                                mMap.addMarker(new MarkerOptions().position(marker).title("Início da Rota")); //Create Marker
                            }

                            if(i == itinerario.positions.size()){
                                mMap.addMarker(new MarkerOptions().position(marker).title("Final da Rota").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
                            }
                            polLineOpt.add(marker);
                            i++; //Increment int
                        }
                        mMap.addPolyline(polLineOpt);
                    }
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // Add a marker in PORTO ALEGRE and move the camera
        LatLng portoAlegre = new LatLng(-30.0277, -51.2287);
        mMap.addMarker(new MarkerOptions().position(portoAlegre).title("Marker in Porto Alegre").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_ORANGE)));
        ...
        if(getIntent().hasExtra("LINHAID")){
            ...
            //DO ELSE -> NO LINHAID
            Toast.makeText(MapsActivity.this, "NO EXTRA NAMED LINHAID", Toast.LENGTH_SHORT);
        }
    }
}
